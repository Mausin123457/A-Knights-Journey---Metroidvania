...
9-8-2026: So a little break turned into a big break, because I was on vacation for a long time. I didn't really do any work on the game while I was on vacation, but before leaving I did add a couple of things. I made a fully working map system for example. It updates whenever a player saves their game at a campfire and it shows a player indicator when walking around. This is definitely not completely finished and polished yet, but it works and it's completely to scale! This system actually didn't really cause too much trouble or time. I also updated the input hints, it now checks for a new device every time a person interacts at a campfire or pauses the game, and I might add some more places where we check the device type, like when hitting a switch for example. Which leads me to my next implementation, a door and switch system. I set that up completely and is persistent in the data, so it saves when you close the game (you obviously have to hit a campfire before it fully saves). So what I will be working on next is the audio system and creating music/sound effects for my game. This is something I have never done before, so this will definitely be a learning curve and will probably cause a lot of headaches, but I am sure that I will figure it out eventually. Most of my updates will probably be about the sound and audio system for a little bit.
